...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  async function login(phone: string, password: string): Promise<AppUser> {
    ...
    if (!appUser) throw new Error('المستخدم غير موجود في قاعدة البيانات');
    ...
    // Set user state immediately so navigate() works right away
    setUser(appUser);
  ...
  async function register(data: RegisterData): Promise<string> {
    ...
    await setDoc(doc(db, 'users', uid), {
      ...
      ...(data.familyHeadId && { familyHeadId: data.familyHeadId }),
